  ...
  getItems = () => {
    ...
    itemList = items.map((item, i) => {
      ...
      // if (item.desc && item.image !== '') {
      	// list = <span data-time={item.time} data-title={item.title} data-image={item.image} data-desc={item.desc} onClick={this.showPreview} key={i}>
				// 	<ItemTextImage time={item.time} title={item.title} desc={item.desc} image={item.image}/></span>;
      	// return list
      //
      // } else if (item.desc === '') {

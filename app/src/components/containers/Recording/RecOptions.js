...
function mapStateToProps(state) {
  return {
    ...
    TopBar_Title: state.TopBar_Title
  }
}
//define actions
...
class RecOptions extends React.Component {
  ...
  componentWillMount() {
    navigator.mediaDevices
      ...
      .then((stream) => this.recControl(stream))
      .catch((err) => {
  ...
  recControl = (stream) => {
    ...
    recorder.onstop = (e) => {
      ...
      let currentDateSort =
        '' + d.getFullYear() + d.getMonth() + d.getDate() + d.getHours() + d.getMinutes() + d.getMilliseconds()
      ...
      firebase
        .database()
        .ref(`users/${firebase.auth().currentUser.uid}/masterNotes`)
        .push({
        ...
        .then((snap) => {
          ...
          let blob = new Blob(this.state.recordedChunks, { type: 'audio/mpeg; codecs=opus' })
                  ...
                  let uploadCompressed = (compressedImage) => {
                    ...
                    mountainsRef.put(compressedImage).then((snapshot) => {
                      ...
                      if (flag === true) {
                        firebase
                          .database()
                          .ref(`users/${firebase.auth().currentUser.uid}/masterNotes/${this.state.newMasterNoteKey}`)
                          .update({ backImg: snapshot.metadata.downloadURLs[0] })
                        flag = false
                      }
  ...
  render() {
    ...
    const StopIcon = styled.img`
      display: ${(props) => (this.props.stopStatus === false ? 'block' : 'none')};
      width: 40px;
      cursor: pointer;
    `
    const PlayIcon = styled.img`
      display: ${(props) => (this.props.playStatus === false ? 'none' : 'block')};
      width: 40px;
      cursor: pointer;
    `
    const PauseIcon = styled.img`
      display: ${(props) => (this.props.pauseStatus === false ? 'block' : 'none')};
      width: 40px;
      cursor: pointer;
    `
    //Template
    return (
      <Wrapper>
        <Left>
          <Icon onClick={this.showNote} src={Note_icon} alt="Note icon" />
        </Left>
        <Center>
          <Top>
